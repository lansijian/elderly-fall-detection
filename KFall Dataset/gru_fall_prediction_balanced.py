...
def evaluate_model(model, test_loader, device):
    ...
    title = 'Confusion Matrix (GRU Model)'  # 混淆矩阵 (GRU模型)
    xlabel = 'Predicted'  # 预测
    ylabel = 'Actual'  # 实际
...
def plot_training_history(history):
    ...
    # 使用英文标签
    loss_title = 'Training and Validation Loss (GRU Model)'  # 训练和验证损失 (GRU模型)
    metrics_title = 'Validation Accuracy and AUC (GRU Model)'  # 验证准确率和AUC (GRU模型)
...
def main():
    """主函数"""
    # 主函数
    print("=== GRU-based Fall Prediction Model with Balanced Data (1:2) ===\n")  # 基于GRU的跌倒预测模型（平衡数据 1:2）
    ...
    print(f"\n=== 训练和评估完成 ===")  # 训练和评估完成
    print(f"最佳模型已保存为 best_model_gru.pth")  # 最佳模型已保存
    print(f"混淆矩阵已保存为 confusion_matrix_gru.png")  # 混淆矩阵已保存
    print(f"训练历史已保存为 training_history_gru.png")  # 训练历史已保存
