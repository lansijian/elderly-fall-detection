...
class CNNLSTM(nn.Module):
    ...
    def forward(self, x):
        ...
        # 转置回来以适应LSTM [batch, channels, seq_len] -> [batch, seq_len, channels]
        x = x.permute(0, 2, 1)
        
        # LSTM处理
        lstm_out, _ = self.lstm(x)  # lstm_out: [batch_size, reduced_seq_len, hidden_size]
        
        # 注意力机制
        attention_weights = self.attention(lstm_out)  # [batch_size, reduced_seq_len, 1]
...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, device, epochs=EPOCHS):
    ...
    for epoch in range(epochs):
        ...
        # 早停检查
        early_stopping(val_auc, model, 'best_model_cnn_lstm.pth')
...
def evaluate_model(model, test_loader, device):
    ...
    title = 'Confusion Matrix (CNN-LSTM)'  # 混淆矩阵 (CNN-LSTM)
    xlabel = 'Predicted'  # 预测
    ylabel = 'Actual'  # 实际
...
def plot_training_history(history):
    ...
    # 使用英文标签
    loss_title = 'Training and Validation Loss (CNN-LSTM)'  # 训练和验证损失 (CNN-LSTM)
    metrics_title = 'Validation Accuracy and AUC (CNN-LSTM)'  # 验证准确率和AUC (CNN-LSTM)
...
def main():
    """主函数"""
    # 主函数
    print("=== CNN-LSTM混合模型跌倒预测 (平衡数据 1:2) ===\n")  # CNN-LSTM混合模型跌倒预测（平衡数据 1:2）
    ...
    print(f"\n=== 训练和评估完成 ===")  # 训练和评估完成
    print(f"最佳模型已保存为 best_model_cnn_lstm.pth")  # 最佳模型已保存
    print(f"混淆矩阵已保存为 confusion_matrix_cnn_lstm.png")  # 混淆矩阵已保存
    print(f"训练历史已保存为 training_history_cnn_lstm.png")  # 训练历史已保存
