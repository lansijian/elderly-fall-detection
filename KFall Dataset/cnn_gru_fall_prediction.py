...
class CNNGRU(nn.Module):
    """
    CNN+GRU混合模型：先使用1D-CNN提取局部模式特征，再用GRU处理时序依赖关系
    """
    ...
    def forward(self, x):
        ...
        # 转置输入以适应卷积层 [batch, seq_len, channels] -> [batch, channels, seq_len]
        x = x.permute(0, 2, 1)
        ...
        # 转置回来以适应GRU [batch, channels, seq_len] -> [batch, seq_len, channels]
        x = x.permute(0, 2, 1)
        
        # GRU处理
        gru_out, _ = self.gru(x)  # gru_out: [batch_size, reduced_seq_len, hidden_size]
        
        # 注意力机制
        attention_weights = self.attention(gru_out)  # [batch_size, reduced_seq_len, 1]
...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, device, epochs=EPOCHS):
    ...
    for epoch in range(epochs):
        ...
        # 早停检查
        early_stopping(val_auc, model, 'best_model_cnn_gru.pth')
...
def evaluate_model(model, test_loader, device):
    ...
    title = 'Confusion Matrix (CNN-GRU)'  # 混淆矩阵 (CNN-GRU)
    xlabel = 'Predicted'  # 预测
    ylabel = 'Actual'  # 实际
    ...
    plt.title(title)
    plt.tight_layout()
    plt.savefig('confusion_matrix_cnn_gru.png')
...
def plot_training_history(history):
    ...
    # 使用英文标签
    loss_title = 'Training and Validation Loss (CNN-GRU)'  # 训练和验证损失 (CNN-GRU)
    metrics_title = 'Validation Accuracy and AUC (CNN-GRU)'  # 验证准确率和AUC (CNN-GRU)
    ...
    plt.tight_layout()
    plt.savefig('training_history_cnn_gru.png')

def main():
    """主函数"""
    # 主函数
    print("=== CNN-GRU混合模型跌倒预测 (平衡数据 1:2) ===\n")  # CNN-GRU混合模型跌倒预测（平衡数据 1:2）
    ...
    # 加载最佳模型
    model.load_state_dict(torch.load('best_model_cnn_gru.pth'))
    ...
    print(f"\n=== 训练和评估完成 ===")  # 训练和评估完成
    print(f"最佳模型已保存为 best_model_cnn_gru.pth")  # 最佳模型已保存
    print(f"混淆矩阵已保存为 confusion_matrix_cnn_gru.png")  # 混淆矩阵已保存
    print(f"训练历史已保存为 training_history_cnn_gru.png")  # 训练历史已保存
