...
def evaluate_model(model, test_loader, device):
    ...
    title = 'Confusion Matrix (Balanced Data 1:2)'  # 混淆矩阵 (平衡数据 1:2)
    xlabel = 'Predicted'  # 预测
    ylabel = 'Actual'  # 实际
    ...
    plt.title(title)
    plt.tight_layout()
    plt.savefig('confusion_matrix_balanced_1to2.png')
...
def plot_training_history(history):
    ...
    # 使用英文标签
    loss_title = 'Training and Validation Loss (Balanced Data 1:2)'  # 训练和验证损失 (平衡数据 1:2)
    metrics_title = 'Validation Accuracy and AUC (Balanced Data 1:2)'  # 验证准确率和AUC (平衡数据 1:2)
    ...
    plt.tight_layout()
    plt.savefig('training_history_balanced_1to2.png')

def main():
    """主函数"""
    # 主函数
    print("=== LSTM-based Fall Prediction Model with Balanced Data (1:2) ===\n")  # 基于LSTM的跌倒预测模型（平衡数据 1:2）
    ...
    print(f"\n=== 训练和评估完成 ===")  # 训练和评估完成
    print(f"最佳模型已保存为 best_model_balanced.pth")  # 最佳模型已保存
    print(f"混淆矩阵已保存为 confusion_matrix_balanced_1to2.png")  # 混淆矩阵已保存
    print(f"训练历史已保存为 training_history_balanced_1to2.png")  # 训练历史已保存
