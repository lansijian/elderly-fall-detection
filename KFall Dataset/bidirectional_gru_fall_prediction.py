...
class BidirectionalGRUFallPredictor(nn.Module):
    # 基于双向GRU的跌倒预测模型
    ...
    def __init__(self, input_size=len(SENSOR_COLS), hidden_size=HIDDEN_SIZE, 
        ...
        """
        # 初始化模型参数
        super(BidirectionalGRUFallPredictor, self).__init__()
        ...
        self.attention = nn.Sequential(
            nn.Linear(hidden_size * 2, hidden_size * 2), # 线性变换，*2因为是双向
            nn.Tanh(), # 激活函数
        ...
        self.fc = nn.Sequential(
            nn.Linear(hidden_size * 2, hidden_size * 2), # 线性变换
            nn.ReLU(), # 激活函数
            nn.Dropout(dropout), # 丢弃
            nn.Linear(hidden_size * 2, hidden_size), # 线性变换
            nn.ReLU(), # 激活函数
    ...
    def forward(self, x):
        ...
        # GRU层
        gru_out, _ = self.gru(x)  # gru_out: (batch_size, seq_len, hidden_size*2)，因为是双向

        # 注意力机制
        attention_weights = self.attention(gru_out)  # (batch_size, seq_len, 1)
        context_vector = torch.sum(attention_weights * gru_out, dim=1)  # (batch_size, hidden_size*2)
...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, device, epochs=EPOCHS):
    ...
    for epoch in range(epochs):
        ...
        # 早停检查
        early_stopping(val_auc, model, 'best_model_bidirectional_gru.pth')
...
def evaluate_model(model, test_loader, device):
    ...
    title = 'Confusion Matrix (Bidirectional GRU)'  # 混淆矩阵 (双向GRU)
    xlabel = 'Predicted'  # 预测
    ylabel = 'Actual'  # 实际
    ...
    plt.title(title)
    plt.tight_layout()
    plt.savefig('confusion_matrix_bidirectional_gru.png')
...
def plot_training_history(history):
    ...
    # 使用英文标签
    loss_title = 'Training and Validation Loss (Bidirectional GRU)'  # 训练和验证损失 (双向GRU)
    metrics_title = 'Validation Accuracy and AUC (Bidirectional GRU)'  # 验证准确率和AUC (双向GRU)
    ...
    plt.tight_layout()
    plt.savefig('training_history_bidirectional_gru.png')

def main():
    """主函数"""
    # 主函数
    print("=== 双向GRU跌倒预测模型 (平衡数据 1:2) ===\n")  # 双向GRU跌倒预测模型（平衡数据 1:2）
    ...
    # 创建模型
    model = BidirectionalGRUFallPredictor()
    ...
    # 加载最佳模型
    model.load_state_dict(torch.load('best_model_bidirectional_gru.pth'))
    ...
    print(f"\n=== 训练和评估完成 ===")  # 训练和评估完成
    print(f"最佳模型已保存为 best_model_bidirectional_gru.pth")  # 最佳模型已保存
    print(f"混淆矩阵已保存为 confusion_matrix_bidirectional_gru.png")  # 混淆矩阵已保存
    print(f"训练历史已保存为 training_history_bidirectional_gru.png")  # 训练历史已保存
